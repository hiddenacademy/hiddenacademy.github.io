Voici ton article avec la méthode directe activée pour Mermaid :
...
graph TD
    subgraph "🔧 Mode Actif"
        A1[🖥️ Client] -->|1. Connexion Contrôle<br/>Port 21| A2[🖥️ Serveur]
        A2 -->|2. Connexion Données<br/>Port 20 → Port Client| A1
        A1 --> A3[⚠️ Problèmes Firewall]
        A3 --> A4[❌ Blocage Connexions Entrantes]
    end
    
    subgraph "🛡️ Mode Passif"
        B1[🖥️ Client] -->|1. Connexion Contrôle<br/>Port 21| B2[🖥️ Serveur]
        B2 -->|2. Fournit Port Données| B1
        B1 -->|3. Connexion Données<br/>Port Client → Port Serveur| B2
        B2 --> B3[✅ Passe les Firewalls]
        B3 --> B4[🎯 Recommandé]
    end
</div>
